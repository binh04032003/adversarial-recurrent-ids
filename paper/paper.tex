...
%Note, however, that f
For successful adversarial samples, the second term in \autoref{eq:carliniWagner} becomes constant and, hence, is no longer relevant for optimization. Hence, $c$ is irrelevant for the achieved distance, but governs if for one sample an adversarial sample is found.
However, when using a large $c$ in \autoref{eq:carliniWagner}, to avoid overly large steps that impede convergence, it is necessary to reduce the gradient descent learning rate. Since the distance term is not affected by $c$, it is furthermore necessary to scale the iteration count inversely with the learning rate. \gls{cw} with a large $c$ therefore needs more time than with a small $c$.
...
\text{ARS} = \frac{1}{\lfloor N/2 \rfloor} \sum\nolimits_{d \in \{d \in \mathcal D \mid d < Q_{0.5}(\mathcal D)\}} d,
\end{equation}
...
\gls{cw} is able to find adversarial samples with minimum distance, but becomes slow for large $c$. Hence, it can be used for finding the \gls{ars} efficiently as follows: 
%Since \gls{cw} becomes slow for large $c$, the \gls{ars} can be efficiently found as follows:
