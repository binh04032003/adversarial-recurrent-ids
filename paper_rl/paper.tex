...
For evaluation, we use the \textit{CIC-IDS-2017} \cite{sharafaldin_toward_2018} and \textit{UNSW-NB15} \cite{moustafa_unsw-nb15:_2015} datasets which each include more than 2 million flows of network data, containing both benign traffic and a large number of different attacks. The distribution of different attacks in the datasets are shown in \autoref{tab:occurrence}.
...
When experimenting with continuous actions, we encountered one practical problem: At the beginning of training, the classifier would still perform badly and not give a useful reward to the actor. Thus the actor would only focus on the sparsity and maximize sparsity, as the classification reward would always be equally bad no matter how many packets are chosen. Thus the actor would learn to let $\mu$ go to infinity as this would increase the reward at the beginning when the interaction between the three components or our \gls{rl} system is still fragile. As a solution, we add a penalty when the actor jumps far behind the last packet of a flow. It only applies to the last chosen packet and penalizes the actor jumping more than one packet behind the last packet. For this, we modify the formula of the sparsity reward (\autoref{eq:R_sparsity}) for the last packet as follows, where \textit{last} refers to the index of the last chosen packet in the flow: 
...
We opt for three sampling families that do not require feedback from higher layers (i.e. sampling rates are constant and not correlated with subsequent analysis): %The underlying principle of each family is summarized as follows:
...
\centering
\begin{tabular}{>{\rowmac}l>{\rowmac}r>{\rowmac}r>{\rowmac}r>{\rowmac}r>{\rowmac}r>{\rowmac}r<{\clearrow}} \toprule
...
\setrow{\bfseries}Sparsity & 0\% & 76.3\% & 76.5\% & 0\% & 83.5\% & 84.5\% \\
\bottomrule
\end{tabular}
\end{table}
...
\autoref{tab:results_tradeoff} shows the influence of the tradeoff parameter $\alpha$ for both datasets. For the columns with a tradeoff of 0.0, we train the classifier regularly with all packets without using \gls{rl}. For tradeoffs 0.1 and 0.5 we train \ours{} using \gls{rl} with continuous actions and keep the tradeoff fixed for the whole training. We train for 8 epochs ($>10$ million flows for both datasets) and then evaluate the resulting neural networks on the test dataset. Results show that the higher the tradeoff, the fewer packets are sampled and also the lower accuracy. The only exception for this is that for tradeoffs $0.1$ and $0.5$ for UNSW-NB15, the accuracy is almost the same or even a little bit higher for the larger tradeoff, which is counterintuitive. We consider this to be coincidence that can happen due to the stochasticity of the training process. 

\todo{Write that the sparsity is higher for UNSW-NB15 because it contains longer flows}

\subsection{Discrete vs.~Continuous actions}

\begin{table}
\caption{Comparing classification metrics per flow with either discrete (20 different actions) or continuous actions for both CIC-IDS-2017 and UNSW-NB15. The tradeoff is always $0.1$.} \label{tab:results_disc_cont}
\centering
\begin{tabular}{>{\rowmac}l>{\rowmac}r>{\rowmac}r>{\rowmac}r>{\rowmac}r<{\clearrow}} \toprule
...
\setrow{\bfseries}Sparsity & 76.3\% & 73.6\% & 83.5\% & 86.6\% \\
\bottomrule
\end{tabular}
\end{table}

\autoref{tab:results_disc_cont} compares continuous and discrete (20 different) actions shows that for CIC-IDS-2017 continuous actions result in a higher accuracy with fewer samples being chosen. Conversely, for UNSW-NB-15, accuracy is the same for both continuous and discrete actions. However, with discrete actions, fewer packets are chosen, which leads to the conclusion that in this case, discrete actions perform better. This leads to the conclusion, that at least for our use case and the two datasets that we consider, there is no consistent difference in performance for continuous and discrete actions. Generally, the advantage of discrete actions is that they are theoretically more powerful as they can compare each of the possible actions with each other, while for continuous actions the only possibility is to increase or decrease the mean of the log-normal distribution, which can results in worse performance. On the other side, with discrete actions, the maximum number of samples to skip is fixed, which we consider a major drawback. Furthermore, training time is potentially higher since, for example, for a discrete action space of 20, all 20 different actions are treated as being completely different while actually there is not a large difference between skipping 18 or 19 packets while there is a big difference between skipping 0 packets compared to skipping 19 packets. This means that for discrete actions, all actions are viewed as being completely independent, which is not true as the reward function considers sparsity, which increases gradually the more packets are skipped and for which similar actions result in similar rewards. 

\subsection{Comparing with other sampling methods}

\begin{table}
\caption{Comparing classification metrics per flow between \gls{rl}-based sampling with continuous actions (tradeoff $0.1$) and alternative strategies for CIC-IDS-2017.} \label{tab:results_others}
\centering
\begin{tabular}{>{\rowmac}l>{\rowmac}r>{\rowmac}r>{\rowmac}r>{\rowmac}r>{\rowmac}r<{\clearrow}} \toprule
& \gls{rl} & random & every $i$th & first $n$ & first $n$ equal \\	\midrule
...
\setrow{\bfseries}Sampled & 76.3\% & 76.3\% & 76.3\% & 76.3\% & 76.3\% \\
\bottomrule
\end{tabular}
\end{table}

Next we compared the \gls{rl} technique usedy by \ours{} with several intuitive sampling strategies we proposed in \autoref{sec:other_sampling_techniques}. 
\autoref{tab:results_others} shows that with the same sparsity, \ours{} achieves the highest accuracy. Not surprisingly, random sampling performs the worst, as it is possible, that a flow is ``unlucky'' and doesn't get enough packets sampled while another one might get more packets than necessary. The best strategy besides the \gls{rl} approach seems to be ``first $n$ equal'' sampling, which matches the conclusion from \cite{hartl_explainability_2019} that the first couple of packets are the most important ones for determining maliciousness of a flow. ``first $n$ equal'' performs better than ``first $n$'' because for ``first $n$'', longer flows will have more packets sampled from them. Conversely, very short flows will get few packets. But since the first packets of a flow determine its maliciousness, the short flows get misclassified because they don't get enough packets sampled. 

\section{Steering}

So far we have always trained \ours{} with a fixed tradeoff parameter $\alpha$. However, there is also the important use case in which an operator wants a certain minimum sparsity, for example 50\%, to limit the required computation power and thus limit cost. 
We thus propose a method that enables steering the tradeoff to achieve a certain wanted sparsity.
\begin{enumerate}
...
\item During deployment, the tradeoff is set to the maximum that was used during training (1 in our case), which results in maximum sparsity. Then, the tradeoff is continuously incremented if the current sparsity is still under the specified minimum. It is important that the tradeoff can never fall below 0 since this is a case that was not encountered during training and which can lead to anomalous behavior. 
\end{enumerate}


%{{"../plots/plot/flows_pred_plots2_outcomes_0_3.pickle_5_DoS - DDoS-DoS Hulk"}
\begin{figure}
\centering
  \includegraphics[width=\columnwidth]{{"img/lstm_module_8.pth_global_tradeoff_1.0_steering_step_size_0.05_steering_target_sparsity_0.5_batches_to_consider_for_steering_500_sampling_rl"}.pdf}
  \caption{Steering the tradeoff with an allowed minimum sparsity of 50\%. The tradeoff is initialized as 1 in the beginning and continuously lowered as long as the sparsity is higher than 50\% for CIC-IDS-2017. The procedure stops if the minimum sparsity is reached or the tradeoff reached its minimum of 0, which is the case here. Sparsity falls from around 74.5\% to 73.5\%.}
  \label{fig:steering}
\end{figure}
...
We saw that the \gls{rl}-based approach with a tradeoff between classification accuracy and sparsity works as expected and that higher tradeoffs resulted in higher sparsity and potentially lower classification performance. Surprisingly, we saw that our solution \ours{} chooses very few packets: Typically $\approx \frac{1}{4}$ for CIC-IDS-2017 and $\approx \frac{1}{5}$ for UNSW-NB-15. 

Furthermore, our results showed that \gls{rl}-based sampling can intelligently choose relevant packets for intrusion detection and outperform non-smart sampling techniques. 
Besides discrete actions for our \gls{rl} system we also implemented continuous ones. A naive implementation did not work as the training was too fragile in the beginning and the \gls{rl} system would diverge and the number of skipped packets would go to infinity. Adding a penalty for jumping behind the end of the sequence proved effective. Comparing with discrete actions, continuous actions seemed to perform on par. This is still an advantage, as for continuous actions the neural network only needs to have two outputs (mean and standard deviation) while for discrete ones it needs many outputs as there are actions. The continuous action framework is thus more lightweight. 
