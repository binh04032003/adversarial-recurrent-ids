...
The results of the sampling part are discussed in this section. We study the sparsity-accuracy tradeoff first, then we evaluate both discrete and continuous actions and, finally, we compare our results with other sampling techniques. Overall, sampling based on \gls{rl} reveals that a high number of packets can be skipped while maintaining high performance. Then, we inspect how the \gls{rl} chooses packets and if some patterns can be spotted. 

For all our experiments we train for at least 8 epochs ($>10$ million flows for both datasets) and then evaluate the resulting neural networks on the test dataset. \autoref{fig:training} shows how during training, accuracy increases quickly in the beginning and then gradually afterwards. At the same time, sparsity gets higher as the \gls{rl} learns which packets can be skipped since they do not influence classification performance. 
...
\autoref{fig:steering} shows the steering in action. We start with a maximum sparsity of ~74.4\% and try to reach 50\%. In order to do so, the steering system start decreasing the tradoff parameter until it reaches 0. It might seem surprising that even with such value, which means that only classification matters and not sparsity at all, a sparsity of 73.5\% is still achieved. We attribute this (1) to the fact that \ours{} learns that some flows may contain packets that do not contribute in determining maliciousness and are skipped since they do not affect the accuracy and (2) the actor still in any case attempts to maximize the entropy of the output which induces skipping packets for fat-tailed distributions. %Nothworthy is the fact that the goal sparsity (50\%) is not reached due to the previous assumptions, however, a decreasing slope is clearly shown in figure 4


\section{Conclusion}
\label{sec:conclusion}
In this paper we try to solve the practical problem of accurate network traffic classification with less computational power. To this end, we propose \ours{} which is a framework for packet sampling based on \gls{rl}.
