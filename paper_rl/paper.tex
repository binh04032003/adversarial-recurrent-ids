...
The critic aims to estimate the future expected average classification performance and the future expected average sparsity that can be achieved at a timestep $t$ (a timestep is a packet). For this, given the current input vector and the state of the LSTM cells $s_t$ and the neural network weights of the critic $\theta_v$, it outputs $v_{\text{classification},t}$ and $v_{\text{sparsity},t}$ at each timestep $t$. It aims to predict the classification performance and sparsity correctly for each timestep by minimizing the following loss function:
\begin{align}
\begin{split}
l_{\text{v},t} =& \left(R_{\text{classification},t} - v_{\text{classification},t}\left(s_t ; \theta_\text{c} \right)\right)^2 \\
+ & \left(R_{\text{sparsity},t} - v_{\text{sparsity},t}\left(s_t ; \theta_\text{c} \right)\right)^2
\end{split}
\end{align}
...
When experimenting with continuous actions, we encountered one practical problem: At the beginning of training, the classifier would still perform badly and would not give the actor a useful reward. As a result, the actor would only focus on sparsity and aim to maximize it, as the classification reward would always be equally bad no matter how many packets are chosen. Thus, the actor would learn to let $\mu$ go to infinity. This would increase the reward at the beginning when the interaction between the three components of our \gls{rl} system is still fragile. As a solution, we add a penalty to the reward calculation when the actor jumps far behind the last flow packet. The modified reward applies only to the last chosen packet and penalizes the actor jumping more than one packet behind the last packet. The sparsity reward formula (\autoref{eq:R_sparsity}) of the last packet is modified as follows, where \textit{last} refers to the index of the last chosen packet in the flow ($t=\textit{last}$):
...
For our experiments, we use the \textit{CIC-IDS-2017} \cite{sharafaldin_toward_2018} and \textit{UNSW-NB15} \cite{moustafa_unsw-nb15_2015} datasets, which both include more than 2 million flows of network data, containing both benign traffic and a large number of different attacks. The distribution of different attacks in the datasets are shown in \autoref{tab:occurrence}.

We use Z-score normalization and a train/test split of 2:1 for both datasets. To speed up experimentation, we cut flows at a maximum length of 20. As Figure \ref{fig:inspectingAll} shows, there are only few flows that exceed that length.
...
\centering
  ...
  \caption{Training with a tradeoff of 0.1 with continuous actions for the CIC-IDS-2017 dataset. Training accuracy increases sharply in the beginning while becoming slower the longer training lasts. Sparsity increases sharply in the beginning and then slower. As the classifier gets better, the \gls{rl} starts skipping more packets.}
  \label{fig:training}
\end{figure}
...
\autoref{tab:results_tradeoff} shows the influence of the tradeoff parameter $\alpha$ for both datasets. For columns with a tradeoff of 0.0, we train the classifier regularly with all packets without using \gls{rl}. For tradeoffs 0.1 and 0.5 we train \ours{} using \gls{rl} with continuous actions and keep the tradeoff constant for the whole training.

Results show that the higher the tradeoff, the less packets are sampled and the lower the accuracy. The only exception to this is that for tradeoffs of $0.1$ and $0.5$ for UNSW-NB15, the accuracy is almost the same or even slightly higher for the larger tradeoff, which is counter-intuitive. We consider this to be a coincidence that can happen because of the stochasticity of the training process.

%\note{blue}{Fares: my suggestion:
%The only exception to this is that for tradeoffs of $0.1$ and $0.5$ for UNSW-NB15, the accuracy is almost the same or even slightly higher for the larger tradeoff, which is counter-intuitive. We assume that due to the nature of the UNSW-NB15 data set (which is highly heterogeneous), fewer packets yield better information (and therefore the memory of the LSTM classifier is cleaner) and hence better classification decision and higher accuracy.
%}
% Max: Could be but it is too speculative in my opinion. From what I saw, neural networks depend quite a lot on the initialization and on luck, so it might just be that it was lucky...

The fact that for UNSW-NB15 sparsity is higher is caused by the fact that it has more packets per flow on average and thus more redundant information that the \gls{rl} learns to skip. %This, on the other hand, holds since almost the same number of packets is sampled from each flow for both datasets (see \autoref{sec:conclusion}).
% Max: I don't understand the sentence commented out above...
...
\autoref{tab:results_others} reveals that \ours{} achieves the highest accuracy with the same sparsity. Not surprisingly, random sampling performs the worst, as it is likely that a flow is ``unfortunate'' and doesn't get enough packets sampled while another one gets more packets or the packets that carry more useful information. The best strategy besides the \gls{rl} approach seems to be ``first $n$ equal'' sampling, which matches the conclusion from \cite{hartl_explainability_2019} that the first couple of packets are the most important ones for determining maliciousness of a flow.
``first $n$ equal'' performs better than ``first $n$'' because for ``first $n$'', longer flows will have more packets sampled from them. Conversely, very short flows will get few packets. For example, with $p=0.2$, a flow of length 20 will get 4 packets while one with 4 packets will get only 1. However, with `first $n$ equal'', the short and the long flow will get the same number of packets. Apparently it is better if all flows get the same number of packets than if long flows get more packets. 
