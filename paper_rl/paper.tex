...
For all our experiments we train for at least 8 epochs ($>10$ million flows for both datasets) and then evaluate the resulting neural networks on the test dataset. \autoref{fig:training} shows how during training, accuracy increases quickly in the beginning and then gradually afterwards. At the same time, sparsity gets higher as the \gls{rl} learns which packets can be skipped since they do not influence classification performance. An important implementation detail is that we always start with the first packet of a flow, meaning that it cannot be skipped. Also, we set the entropy parameter to $0.01$. 

Opposed to the behavior during training, we do not randomly sample from the probability distribution of the actor during testing but instead take the mean of the distribution for the continuous case and the mode for the discrete one. This is reasonable and best practice \cite{mnih_asynchronous_2016} since during testing we do not aim to explore the possible choices anymore since they were already learned during training: Only the optimal choice, which was learned, should be applied and no exploration is necessary since the actor does not train anymore. 
...
Furthermore, due to the fact that UNSW-NB15 contains more packets per flow, its sparsity is higher. %This, on the other hand, holds since almost the same number of packets is sampled from each flow for both datasets (see \autoref{sec:conclusion}).
...
Finally, we implement a steering method with which a certain sparsity threshold can be specified which limits the computational expenses of an operator. 
