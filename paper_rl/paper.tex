...
\autoref{tab:results_tradeoff} shows the influence of the tradeoff parameter $\alpha$ for both datasets. For columns with a tradeoff of 0.0, we train the classifier regularly with all packets without using \gls{rl}. For tradeoffs 0.1 and 0.5 we train \ours{} using \gls{rl} with continuous actions and keep the tradeoff constant for the whole training. We train for 8 epochs ($>10$ million flows for both datasets) and then evaluate the resulting neural networks on the test dataset.\par
...
\label{sec:conclusion}
 In this paper we try to solve the practical problem of accurate network traffic classification with less effort. To this end, we propose \ours{} which is a framework for packet sampling based on \gls{rl}.
