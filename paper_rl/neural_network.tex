...
\newcommand{\backgroundnc}[4]{%%
  \begin{pgfonlayer}{background}%
    ...
    \path[rounded corners, draw=black!50]%
      (a1) rectangle (a2);%
    \path (a1.east |- a1.south)+(2.5,0.25) node (u1)[anchor=south west]%
      {\textit{#3}};%
  \end{pgfonlayer}}%
...
\begin{scope}
\node [input place] (s1) [label={[align=center]left: Sample (16)}] {};
...
\node [place] (s13) [right of=s21, label={[align=center]above: Linear}] {};
